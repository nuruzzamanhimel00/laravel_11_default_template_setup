                ...
                <form action="{{ route('promotions.update', $promotion->id) }}" method="post" enctype="multipart/form-data">
                    ...
                    <div class="row" >
                        <div class="form-group col-xl-6 col-lg-6 p-2">
                            ...
                            <select class="form-control" name="target_type" id="target_type" required>
                                ...
                                {{ old('target_type', $target_type) == \App\Models\User::TYPE_REGULAR_USER ? 'selected' : '' }}>
                                    {{ ucwords(\App\Models\User::TYPE_REGULAR_USER) }}</option>

                                <option value="{{ \App\Models\User::TYPE_RESTAURANT }}"
                                {{ old('target_type', $target_type) == \App\Models\User::TYPE_RESTAURANT ? 'selected' : '' }}>
                        ...
                        <div class="form-group col-xl-6 col-lg-6 p-2">
                            ...
                            <input type="date" name="end_date" class="form-control" :min="min_date"
                                x-model="end_date" required placeholder="Enter End Date" @change="loadData">
                    ...
                    <template x-if=" is_load_data == true">
                        <div class="form-group col-xl-6 col-lg-6 p-2">
                            ...
                            <select class="form-control" x-model="applied_for" name="applied_for" @change="select2Initial()" required>
                                <option selected>Select One</option>
                                @foreach (\App\Models\Promotion::APPLICABLE_TYPES as $applicable)

                                <option value="{{$applicable}}" {{old('applied_for') == $applicable ? 'selected' : ''}} >{{ucwords($applicable)}}</option>
                                @endforeach
                    ...
                    <template x-if="applied_for == '{{\App\Models\Promotion::APPLICABLE_PRODUCTS}}' && is_load_data == true">
                        <div class="form-group col-xl-6 col-lg-6 p-2">
                            ...
                            <select
                                ...
                                multiple="multiple"
                                required
                                x-data="{ products: products, applied_for: applied_for, appliedProducts:appliedProducts}">
                                <template x-for="product in products" :key="product.id">
                                    <option
                                        :value="product.id"
                                        :selected="applied_for == 'products'  && appliedProducts.includes(product.id)"
                                        x-text="product.name">
                                    </option>
                                </template>
                            </select>
                    ...
                    <template x-if="applied_for == '{{\App\Models\Promotion::APPLICABLE_CATEGORIES}}' && is_load_data == true">
                        <div class="form-group col-xl-6 col-lg-6 p-2">
                            ...
                            <select
                                ...
                                multiple="multiple"
                                required
                                x-data="{ categories: categories, applied_for: applied_for, appliedCategories:appliedCategories}">
                                <template x-for="category in categories" :key="category.id">
                                    <option
                                        :value="category.id"
                                        :selected="applied_for == 'categories' && appliedCategories.includes(category.id)"
                                        x-text="category.name">
                                    </option>
                                </template>
                            </select>

                            <small class="text-danger">Products already used in this promotion are not eligible for reapplication.</small>
                        ...
                        <div class="form-group col-xl-6 col-lg-6 p-2">
                            ...
                            <select class="form-control" name="offer_type" required>
                                ...
                                <option value="{{$type}}" {{old('offer_type',$promotion->offer_type) == $type ? 'selected' : '' }}>{{ucwords($type)}}</option>
                                @endforeach
                        ...
                        <div class="form-group col-xl-6 col-lg-6 p-2">
                            ...
                            <input type="number"  min="1" name="offer_value" class="form-control" placeholder="Enter Offer Value" required
                                value="{{ old('offer_value',$promotion->offer_value) }}">
                        ...
                        <div class="form-group col-xl-4 col-lg-4 p-2">
                            ...
                            <div class="form-check form-switch">
                                <input
                                    ...
                                    name="in_homepage"
                                    value="1"
                                    {{$promotion->in_homepage == 1 ? 'checked' : ''}}

                                >
                                <label class="form-check-label" for="in_homepage">
                                    {{ old('in_homepage',$promotion->in_homepage) ? 'Enabled' : 'Disabled' }}
                                </label>
                            </div>
                            @error('in_homepage')
                                <p class="text-danger">{{ $message }}</p>
...
<script>
    $(document).ready(function() {
        ...
        //     let value = $(this).val();
        //     window.location.href= "{{route('promotions.edit', $promotion->id)}}?target_type="+value;
        // })
    });
  </script>
 <script>
    document.addEventListener('alpine:init', () => {
        Alpine.data('promotionData', () => ({
            ...
            appliedProducts: @json($promotion->applied_for ===  \App\Models\Promotion::APPLICABLE_CATEGORIES ? [] : $appliedProducts),
            appliedCategories: @json($promotion->applied_for ===  \App\Models\Promotion::APPLICABLE_PRODUCTS  ? [] : $appliedCategories),
            start_date: "{{ old('start_date', \Carbon\Carbon::parse($promotion->start_date)->format('Y-m-d')) }}",
            end_date: "{{ old('end_date', \Carbon\Carbon::parse($promotion->end_date)->format('Y-m-d')) }}",
            min_date: "{{ \Carbon\Carbon::parse($promotion->start_date)->format('Y-m-d') }}",
            applied_for: "{{ old('applied_for', $promotion->applied_for) }}",
            is_load_data: true,
            target_type: "{{ old('target_type', $promotion->target_type) }}",
            categories: [],
            products: [],
            applied_for_ids: [],

            applicableFor(event) {
                this.applied_for = event.target.value;
                this.select2Initial();
            },
