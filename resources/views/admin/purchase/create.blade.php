    ...
    <div class="card">
        <div class="card-body">
            ...
            <form action="{{ route('purchases.store') }}" method="post" enctype="multipart/form-data"
                ...
                @csrf
                {{-- <h4 class="header-title">{{ __('Add Purchase') }}</h4> --}}
                <div class="row">
                    <div class="form-group col-xl-4 col-lg-4 p-2">
                        <label>{{ __('Select Warehouses') }} <span class="error">*</span></label>
                        <div>
                            @if ($warehouses)
                            <select name="warehouse_id" class="form-control select2" required id="warehouse_id">
                                <option value="" selected disabled>{{ __('Select Warehouse') }}</option>
                                @foreach ($warehouses as $warehouse)
                                <option value=" {{ $warehouse->id }}"
                                    {{$warehouse->id == $warehouse_id ? 'selected' : ''}}>{{ $warehouse->name }}
                                </option>
                                @endforeach
                            </select>
                            @endif
...
<div class="form-group col-xl-12 col-lg-12 p-2">
    <label for="short_address">Short address (if you are not fill up this above address then you can fill this short
        address)</label>
    <textarea class="form-control" rows="5" id="short_address" name="short_address"></textarea>
