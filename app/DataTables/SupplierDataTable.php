...
class SupplierDataTable extends DataTable
{
        ...
        return datatables()
            ...
            ->addColumn('action', function ($item) {
                ...
                if (auth()->user()->can('Show Supplier')) {
                    $buttons .= '<li><a class="dropdown-item" href="' . route('suppliers.show', $item->id) . '" title="Show"><i class="fa fa-eye"></i> ' . __('Show') . '</a></li>';
                }
                if (auth()->user()->can('Edit Supplier')) {
                    $buttons .= '<li><a class="dropdown-item" href="' . route('suppliers.edit', $item->id) . '" title="Edit"><i class="mdi mdi-square-edit-outline"></i>' . __('Edit') . '</a></li>';
                }
                if (auth()->user()->can('Delete Supplier')) {
                    $buttons .= '<form action="' . route('suppliers.destroy', $item->id) . '"  id="delete-form-' . $item->id . '" method="post">
    ...
    public function query(User $model)
    {
        ...
        ->with(['supplier:id,company,user_id'])
        ->select(['users.id', 'users.first_name', 'users.email', 'users.phone', 'users.status', 'users.created_at', 'users.avatar'])
        ->latest();
    ...
    protected function getColumns()
    {
        $columns = [
            ...
            Column::make('supplier.company', 'supplier.company')->title(__('Company')),
