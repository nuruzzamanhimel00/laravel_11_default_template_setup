...
class PurchaseReceiveDataTable extends DataTable
{
    ...
    public function dataTable($query)
    {
        return datatables()
            ...
            ->addColumn('action', function ($item) {
                $buttons = '';
                if (auth()->user()->can('Show Receive Purchase')) {
                    $buttons .= '<li><a class="dropdown-item" href="' . route('purchases.receive.show', $item->id) . '" title="Edit"><i class="mdi mdi-eye mr-2"></i> ' . __('Show') . '</a></li>';
                }
            ...
            ->addColumn('total_product', function ($item) {
                return $item->purchase_receive_items->count();
            })
    ...
    public function query(PurchaseReceive $model)
    {
        ...
        ->with(['purchase.supplier'=> function($query){
            ...
            $query->select(['users.id','users.first_name','users.email','users.phone','users.type']);
        },'purchase.warehouse'=> function($query){
            $query->select(['warehouses.id','warehouses.name']);
        },'purchase'])

        ->withCount(['purchase_receive_items'])
        ...
        ->orderBy('purchase_receives.id','desc');
    ...
    protected function getColumns()
    {
        ...
        $columns = [
            ...
            Column::make('receive_date', 'receive_date')->title(__(' Date')),
    ...
    {
        return 'PurchaseReceive_' . date('YmdHis');
    }
