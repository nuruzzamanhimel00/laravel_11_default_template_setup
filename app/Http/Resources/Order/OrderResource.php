...
class OrderResource extends JsonResource
{
    ...
    public function toArray(Request $request): array
    {
        ...
        foreach ($optionalFields as $field) {
            ...
            if (!is_null($field)) {
                if(in_array($field, ['tax_amount', 'discount_amount', 'global_discount','sub_total','total']) && isset($this->{$field})) {
                    $data[$field] = addCurrency(ceil($this->{$field}));
        ...
        if (!is_null($this->total)) {
            $data['price'] = addCurrency(ceil($this->total));
        }
